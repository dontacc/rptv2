            ...
            for x in queryset:
                for a in x:
                    return HttpResponseRedirect(
                        redirect_to='https://rptv2.herokuapp.com{}'.format(reverse('verify', kwargs={'user_id': str(a)})))
    ...
    def put(self, request, *args, **kwargs):
        ...
        for sentcode in dbcode:
            if sentcode == yourcode:
                return HttpResponseRedirect(
                    redirect_to='https://rptv2.herokuapp.com{}'.format(reverse('password_reset_confirm')))
            else:
                return HttpResponseRedirect(
                    redirect_to='https://rptv2.herokuapp.com{}'.format(reverse('rest_password_reset')))
